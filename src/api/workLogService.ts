...
export const saveWorkLog = (log: WorkLogData): void => {
  ...
  // Dispatch custom event for work log updates
  window.dispatchEvent(new CustomEvent("workLogUpdate"));
};
...
export const updateWorkLog = (
  id: string,
  updates: Partial<Omit<WorkLogData, "id">>
): WorkLogData | null => {
  const allLogs = getAllWorkLogs();
...
export const updateTotalWorkTime = (duration: number): void => {
  ...
  window.dispatchEvent(
    new CustomEvent("totalWorkTimeUpdate", {
